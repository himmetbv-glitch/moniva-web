...
export async function POST(
  ...
  if (!post) return Response.json({ error: "Haber bulunamadı." }, { status: 404 });
  ...
  const key = `news/${id}/${randomUUID()}.${ext}`;
  try {
  ...
  await prisma.newsPost.update({ where: { id }, data: { coverImage: url } });

  // Eski kapağı sil
  if (post.coverImage) {
    const oldKey = keyFromAssetUrl(post.coverImage);
  ...
  return Response.json({ coverImage: url });
}
