...
export default async function KariyerPage() {
  const [content, settings] = await Promise.all([getCareersContent(), getSettings()]);
    ...
    <>
      ...
      <main className="careers-page">
        <nav className="cr-crumb">
          ...
          <span>/</span>
          <span className="cr-crumb__cur">{info?.crumbLabel ?? "Kariyer"}</span>
        </nav>

        <section className="cr-apply">
          {info && <CareersInfo d={info} />}
...
function CareersInfo({ d }: { d: CareersInfoData }) {
  return (
